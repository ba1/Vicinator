...
def parse_args():
    ...
    o_group.add_argument(
        ...
        type=argparse.FileType("rt"),
        default=None,
        help="Attempts to replace genome accessions in the outputs with a replacement string. Requires a two-column map file formatted like so: 'genome file accession' <tab> 'replacement string'. The replacement will automatically be cut to a maximum of 30 chars.",
    )
...
class Genome:
    ...
    # def getTextRepresentation(self, basket_list):
    #
    #     from collections import OrderedDict
    #
    #     basket_list = OrderedDict()
    #
    #     basket_list[0] = ["|"]
    #
...
def parseGFF3(gff3_path):
    ...
    ft_df = ft_df[~ft_df["feature"].isin(["exon", "transcript", "mRNA"])] #filtering out irrelevant lines
...
def compare_taxon(t, window_ogs, ogtable, center, extension_mask, label_map):
    ...
    if g:
        # runprofile.enable()
        #TODO: Represent in columns, seperate text output from visualization
        #number_of_cols = len(extension_mask)*3  + len(extension_mask)-1+2
