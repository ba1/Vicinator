...
class Genome:
    ...
    def annotWindowHOGs(self, window_ogs, ogtable, center_strand):
        pos_dict = dict()
        pruned_ogtable = ogtable[ogtable.index.isin([self.name], level=0)]
        #pruned_ogtable.columns = pruned_ogtable.columns.get_level_values(0)
        pruned_ogtable.reset_index(level=pruned_ogtable.index.names, inplace=True)
        pruned_ogtable.drop(['Taxon'], axis=1, inplace=True)
        pruned_ogtable.set_index('PA', inplace=True)
        pruned_ogtable = pruned_ogtable[pruned_ogtable["OG"].isin(window_ogs)] #drop all non-window OGs
        pa2og_dict = pruned_ogtable.to_dict()["OG"]

        for hog in window_ogs:
            if hog:
                pos_dict[hog] = window_ogs.index(hog) - int(len(window_ogs)/2)
        ...
        self.feature_df["OG"] = self.feature_df["product_accession"].map(pa2og_dict)
    ...
    def orderContigs(self):
        ...
        df = self.feature_df[self.feature_df['class'] == 'with_protein'].groupby('genomic_accession')['window_position'].max()
    ...
    def findProteinAccessionIndices(self, protein_accession):
        ...
        indices = self.feature_df[self.feature_df['product_accession'].str.contains(protein_accession).fillna(False)].index.values
        return indices

    def findProteinAccessionsInWindow(self, k, center):
        ...
        filter1 = self.feature_df['class'] == 'with_protein'
    ...
    def __repr__(self, label_map=None):
...
def profile_genome(t, window_ogs, ogtable, center):
    ...
    g.annotWindowHOGs(window_ogs, ogtable, center.loc["strand"])
    try:
...
def main():
    ...
    if len(center_index_list) > 1:
        ...
        logging.critical("Given center protein accession {} not found reference feature file {}.".format(args.centerprotein_accession,
    ...
    window = ref_g.findProteinAccessionsInWindow(k, center)
    ...
    print('Window reference OGs:',
              [og if (type(og) == str) else '-' for og in window_ogs])  # +1 to correct for 0-offset in ogmatrix
    ...
    for t in taxon_feature_files:
        ...
        g = profile_genome(t, window_ogs, ogtable, center)

        #runprofile.enable()
        res = g.__repr__(label_map=label_map)
